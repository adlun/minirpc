...
import com.adlun.minirpc.protocol.protocol.MsgStatus;
...
public class RpcRequestHandler extends SimpleChannelInboundHandler<MiniRpcProtocol<MiniRpcRequest>> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, MiniRpcProtocol<MiniRpcRequest> protocol) {
        RpcRequestProcessor.submitRequest(() -> {
            ...
            try {
                ...
                header.setStatus(MsgStatus.FAILED.getCode());
                response.setMessage(throwable.toString());
                log.error("process request {} error", header.getMsgId(), throwable);
            }
    ...
    private Object handle(MiniRpcRequest request) throws Throwable {
        ...
        if (serviceBean == null) {
            throw new RuntimeException(String.format("service not exist: %s:%s:%s", request.getServiceName(), request.getServiceVersion(), request.getServiceGroup()));
        }
